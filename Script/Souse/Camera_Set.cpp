...
void Camera_Set::Start()
{
	...
	/*
	transform->Set_position(10, -70, 15);
	transform->Set_parent(player_transform);
	Debug::Log(transform->Get_localPosition());
	Debug::Log(transform->Get_localEulerAngles());
	Debug::Log(transform->Get_localScale());
	Debug::Log(transform->Get_position());
	Debug::Log(transform->Get_eulerAngles());
	Debug::Log(transform->Get_scale());
	*/
}
